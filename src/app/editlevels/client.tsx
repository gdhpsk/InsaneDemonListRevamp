...
export default function EditLevels({ authData, levels, leaderboards, packs }: info) {
    ...
    let [deletingInfo, setDeletingInfo] = useState({ reason: "", levelId: { valid: false, info: { name: "", publisher: "", position: 0 }, text: "" } })
                                ...
                                {record !== null ? <Box>
                                    <TextFieldRoot mt="4">
                                        ...
                                        <TextFieldInput defaultValue={level.list[record].player.name} placeholder="Player Name..." id="player" onClick={(e) => {
                                            ...
                                            setLevel({
                                                ...level, list: level.list.map((x: any, i: number) => {
                                    ...
                                    <DialogRoot>
                                        ...
                                        <DialogContent>
                                            ...
                                            <TextFieldRoot>
                                                ...
                                                <TextFieldInput id='reason' onChange={e => setDeletingInfo({ reason: e.target.value, levelId: deletingInfo.levelId })}></TextFieldInput>
                                            </TextFieldRoot>
                                            {deletingInfo.reason ? <><br></br>
                                                <TextFieldRoot>
                                                    <TextFieldSlot>Level ID</TextFieldSlot>
                                                    <TextFieldInput id='reason' onChange={async e => {
                                                        setDeletingInfo({ reason: deletingInfo.reason, levelId: { ...deletingInfo.levelId, text: e.target.value } })
                                                        let req = await fetch(`https://api.aredl.net/api/aredl/levels/${e.target.value}`)
                                                        if (!req.ok) return setDeletingInfo({ reason: deletingInfo.reason, levelId: { ...deletingInfo.levelId, valid: false } })
                                                        let level = await req.json()
                                                        setDeletingInfo({ reason: deletingInfo.reason, levelId: { valid: true, info: { name: level.name, publisher: level.publisher.global_name, position: level.position }, text: e.target.value } })
                                                    }} placeholder="This level must be on the AREDL"></TextFieldInput>
                                                </TextFieldRoot><br></br>
                                                {deletingInfo.levelId.valid ? <CalloutRoot color={"green"}>
                                                    <CalloutIcon>
                                                        <CheckCircledIcon style={{ scale: 1.5 }}></CheckCircledIcon>
                                                    </CalloutIcon>
                                                    <CalloutText size="3" ml="-1">Found Level: <a href={`https://aredl.net/list/${deletingInfo.levelId.text}`} target="_blank" style={{ color: "lightblue" }}>{'#'}{deletingInfo.levelId.info.position} - {deletingInfo.levelId.info.name} by {deletingInfo.levelId.info.publisher}</a></CalloutText>
                                                </CalloutRoot> : <CalloutRoot color={"red"}>
                                                    <CalloutIcon>
                                                        <CrossCircledIcon style={{ scale: 1.5 }}></CrossCircledIcon>
                                                    </CalloutIcon>
                                                    <CalloutText size="3" ml="-1">Could not find this level on the AREDL.</CalloutText>
                                                </CalloutRoot>}</> : ""}
                                            <br></br>
                                            <DialogClose>
                                                <Flex gap="9" justify='center'>
                                                    <Button color='red' size='3' onClick={async _ => {
                                                        ...
                                                        let obj = {
                                                            id: level.id,
                                                            removalReason: deletingInfo.reason,
                                                            levelId: deletingInfo.levelId.text
                                                        }
                                                        setError({ color: "blue", message: "Loading..." })
                                                        let req = await fetch("/api/level/" + level.id, {
                                                            method: "DELETE",
