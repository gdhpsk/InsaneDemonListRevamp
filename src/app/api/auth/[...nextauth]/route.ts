...
import NextAuth, { NextAuthOptions, Theme, getServerSession } from "next-auth"
    ...
    providers: [
       Credentials({
        ...
        async authorize(credentials, req) {
            if(!["login", "signup"].includes(credentials?.type || "")) throw new Error("Please include a valid sign in type!")
            ...
            if(!credentials?.password) {
                ...
                let correctPass = await bcrypt.compare(credentials.password, exists.password)
                if(!correctPass) {
